...
%
% Annie Wong, December 2009
% Breck Owens, October 2006
%--------------------------------------------------------------------------

%pn_float_dir='uw/';
%pn_float_name='R1900271';
...
% replaced above with following - Willa
filename=strcat(po_system_configuration.FLOAT_SOURCE_DIRECTORY, pn_float_name, po_system_configuration.FLOAT_SOURCE_POSTFIX );
...
% replaced above with following - Willa
filename=fullfile( po_system_configuration.FLOAT_MAPPED_DIRECTORY, ...
  strcat( po_system_configuration.FLOAT_MAPPED_PREFIX, pn_float_name, ...
  po_system_configuration.FLOAT_MAPPED_POSTFIX ) );
lo_float_mapped_data = load( filename ) ;
...
% replaced above with following - Willa
lo_float_calib_data = load (fullfile( ...
  po_system_configuration.FLOAT_CALIB_DIRECTORY, ...
  strcat( po_system_configuration.FLOAT_CALIB_PREFIX, pn_float_name, ...
...
% replaced above with following - Willa
lo_float_calseries = load( fullfile(  ...
  po_system_configuration.FLOAT_CALIB_DIRECTORY, ...
  strcat( po_system_configuration.FLOAT_CALSERIES_PREFIX, pn_float_name, ...
  po_system_configuration.FLOAT_MAPPED_POSTFIX ) ) ) ;
...
%   strcat( po_system_configuration.FLOAT_CALIB_PREFIX, pn_float_name,
%   po_system_configuration.FLOAT_CALIB_POSTFIX ) ),'-regexp','^sta');

% replaced above with following - Willa
lo_float_cal = load( fullfile( ...
  po_system_configuration.FLOAT_CALIB_DIRECTORY, ...
  strcat( po_system_configuration.FLOAT_CALIB_PREFIX, pn_float_name, ...
   po_system_configuration.FLOAT_CALIB_POSTFIX ) ),'-regexp','^sta');
...
if(isempty(selected_hist)==0) % if there are historical data
  x=selected_hist(:,1);
  y=selected_hist(:,2);
  ii=find(x>280); %enable plot continuity when data are between 280E (Drake Passage) and 80E;
  jj=find(x<80);
  if(isempty(ii)==0&isempty(jj)==0) % if some points are east of 0, and some are west of 0
    x(jj)=x(jj)+360;
  end
  if( isempty(find(x>360))==0 | (max(LONG)+30)>360 | (min(LONG)-30)<0 )
    mm=find(coastdata_x<80);
    coastdata_x(mm)=coastdata_x(mm)+360;
    nn=find(coastdata_x<81|coastdata_x>439); %remove coastline wraparound
    coastdata_x(nn)=NaN;
    ll=find(LONG<80);
    LONG(ll)=LONG(ll)+360;
  end
...
if(isempty(selected_hist)==1) % if there are no historical data
  kk=find(LONG>280); %enable plot continuity when float is between 280E (Drake Passage) and 80E
  ll=find(LONG<80);
  if(isempty(kk)==0&isempty(ll)==0) % if some points are east of 0, and some are west of 0
    LONG(ll)=LONG(ll)+360;
  end
  if( isempty(find(LONG>360))==0 | (max(LONG)+30)>360 | (min(LONG)-30)<0 )
    mm=find(coastdata_x<80);
    coastdata_x(mm)=coastdata_x(mm)+360;
    nn=find(coastdata_x<81|coastdata_x>439); %remove coastline wraparound
    coastdata_x(nn)=NaN;
...
drawnow
% Willa added plotfile = ....
plotfile=strcat( po_system_configuration.FLOAT_PLOTS_DIRECTORY,pn_float_dir, ...
       po_system_configuration.FLOAT_PLOTS_SUBDIR, pn_float_name);
file_name_save=strcat( po_system_configuration.FLOAT_PLOTS_DIRECTORY,pn_float_dir, ...
       po_system_configuration.FLOAT_PLOTS_SUBDIR,'ow_plot_info.mat');

set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
print('-dpng ', strcat( plotfile, '_1.png'));
...
ok = [];
for k=1:length(jj)
  if ~isempty(find(isfinite(SAL(:,jj))))
      ok = [ok jj(k)];
  end
end
jj = ok;
...
set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
print('-dpng ', strcat( plotfile, '_2.png'));
...
% Willa added next 3 lines, or they are not defined
sta_SAL = lo_float_calib_data.sta_SAL;
sta_mean = lo_float_calib_data.sta_mean;
sta_SAL_err = lo_float_calib_data.sta_SAL_err; 
...
% Willa added "single" to convert xmax to a real number.  Otherwise, it will pass the array as integer
% it will have problem when ymin=0.995 ymax=1.005, it will become
% ymin=1 ymax=1, then it axis() function won't work
axis([ 0, single(max(PROFILE_NO)+1), min([pcond_factor-pcond_factor_err,1])-.0004, max([pcond_factor+pcond_factor_err,1])+.0004 ])
set(gca,'FontSize',12)
...
plot(PROFILE_NO(ok), avg_Staoffset(ok), 'r-');
% JML 5-5-2016 save data 

ave_hist_sal=avg_Staoffset;
ave_fit=avg_Soffset;
ave_fit_error=avg_Soffset_err;
ave_prof_no=PROFILE_NO;



eval(['save ',file_name_save,' ave_hist_sal ave_fit ave_fit_error ave_prof_no'])
% Willa added single below
axis([ 0, single(max(PROFILE_NO)+1), min([avg_Soffset-avg_Soffset_err,0])-.02, max([avg_Soffset+avg_Soffset_err,0])+.02 ])
...
drawnow
set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
print('-dpng ', strcat( plotfile, '_3.png'));
...
set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
print('-dpng ', strcat( plotfile, '_4.png'));
...
print('-dpng ', strcat( plotfile, '_5.png'));
...
if(isempty(Sint)==0)
  ...
  if isfinite(SMIN) & isfinite(SMAX)
    axis([0, single(max(PROFILE_NO))+1, SMIN-.05, SMAX+.05 ])
  end
  set(gca,'FontSize',12)
...
drawnow
set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
print('-dpng ', strcat( plotfile, '_6.png'));
...
drawnow
set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.5,8,10]);
print('-dpng ', strcat( plotfile, '_7.png'));
...
print('-dpng ', strcat( plotfile, '_8.png'));
